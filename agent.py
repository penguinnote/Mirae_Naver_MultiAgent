...
# ── 개방형 상품 추천 질의 판정 ────────────────────────────────────────
#
# 실측 근거(2026-09-04, 배포본 9회): "좋은 연금 상품 하나 추천해 주세요" 류에
# 규칙 5-b가 요구하는 ①역질문이 9/9 빠졌고, 투자권유 면책도 9/9 없었으며,
# 수수료 DB를 조회하지 않은 채 총보수 수치를 제시한 회차가 있었다.
# 규칙 5-b는 이미 존재하고 내용도 정확하지만 3천 자 프롬프트 뒤쪽에 묻힌다.
#
# 전역 SYSTEM_PROMPT에 규칙을 더하는 방식은 세 번 시도해 세 번 다 다른 문항을
# 무너뜨렸다(STEP 7-B·8-C·10-E가 각각 EQ3-M2를 71%→0~17%). 그래서 여기서는
# **해당 문항에서만** compose의 user_suffix에 주입한다 — STEP 1의 sql_empty
# 가드와 같은 패턴이고, 그 패턴은 회귀를 내지 않았다.
#
# 게이트는 동사가 아니라 **펀드 지정 여부**로 가른다. "골라주세요"가 개방형
# 추천(killing camp H-18)과 지정 펀드 비교(V4S-P09) 양쪽에 쓰이기 때문이다.
# 질문에 알려진 펀드명·펀드코드·클래스 코드가 있으면 비교·조회 질의로 보고
# 개입하지 않는다. 오프라인 스캔: 전체 325문항 중 1건(H-18)만 통과하고
# 평가 50·진단 28은 0건이다.

_RECO_ASK_RE = re.compile(r"추천|골라|고르|어떤 상품|뭐가 좋|좋은 상품|무슨 상품")
_RECO_DOMAIN_RE = re.compile(r"연금|펀드|상품|IRP|퇴직|투자")
_RECO_COND_RE = re.compile(
    r"안정|공격|원금|위험|성향|손실|\d+\s*년|\d+\s*세|\d+\s*살"
    r"|연금저축|IRP|퇴직연금|DC|DB|만원|억|은퇴|노후")


def _reco_kind(question: str) -> str | None:
    """개방형 추천 질의면 '조건있음'/'조건없음', 아니면 None."""
    q = question or ""
    if not (_RECO_ASK_RE.search(q) and _RECO_DOMAIN_RE.search(q)):
        return None
    # 특정 상품을 지목한 질문은 비교·조회다. 개방형 추천이 아니다.
    if (_find_known_funds(q) or FUND_CODE_RE.search(q)
            or _CLASS_CODE_RE.search(q)):
        return None
    return "조건있음" if _RECO_COND_RE.search(q) else "조건없음"


# 추천성 답변 끝에 코드가 박는 정형문. 생성에 맡기면 9/9 빠졌다.
_RECO_DISCLAIMER = (
    "※ 위 내용은 제공된 자료에 근거한 일반 정보이며 특정 상품의 투자권유가 "
    "아닙니다. 투자 판단과 그 결과는 투자자 본인에게 귀속되고 펀드는 원금 "
    "손실이 발생할 수 있습니다. 가입 전 투자설명서를 확인하시고, 미래에셋증권 "
    "연금자산관리센터(1588-5577) 또는 영업점에서 본인 상황에 맞는 상담을 "
    "받으시기 바랍니다."
)
...
[규칙 1-c — 근거에 없는 해석·추측을 덧붙이지 않는다]  ★
근거의 사실을 옮긴 뒤, 그 사실에 대한 원인 추정·가능성 언급·상황 판단을
근거 없이 덧붙이지 마십시오.
    나쁨: 같은 항목에 두 값이 기재된 근거를 옮기고 "차이는 반올림 때문일 수
          있습니다"라고 덧붙임 (근거에 그런 설명이 없다)
    나쁨: 근거가 업무 가능 시간대만 적었는데 "현재 시각이 지났다면 불가능할 수
          있습니다"처럼 시각 판단을 지어 붙임
    좋음: 근거의 값·시간대를 그대로 옮기고, 해석이 필요하면 근거에 있는
          설명만 인용한다.
...
[규칙 2-e — 질문의 상품명·제도명이 근거에 없으면 그 사실부터 밝힌다]  ★
질문이 부르는 상품명·제도명이 근거 자료 어디에도 확인되지 않으면, 비슷한
개념으로 바로 답하지 말고 **먼저 그 이름이 확인되지 않는다고 밝히십시오.**
그 다음 유사한 제도·상품이 있으면 그것을 설명합니다.
    나쁨: 존재하지 않는 상품명을 지적 없이 비슷한 제도 설명으로 조용히 치환
    좋음: "'○○'이라는 상품명은 제공된 자료에서 확인되지 않습니다. 유사한
           제도로 △△이 있으며, …"
...
[규칙 2-f — 연금계좌 세액공제 한도의 구조]  ★
근거를 읽을 때 아래 구조를 기준으로 삼으십시오. 근거 문서에 명시된 사실입니다.
  · 연금저축 단독 세액공제 납입한도: 연 600만원
  · 연금저축 + IRP 합산 세액공제 납입한도: 연 900만원
  · IRP 자체에는 별도의 서브한도가 없습니다. 연금저축에 납입하지 않았다면
    IRP만으로 900만원 전액이 공제 대상입니다.
  · 소득에 따라 달라지는 것은 **한도가 아니라 세액공제율**입니다
    (총급여 5,500만원 이하 16.5%, 초과 13.2% / 종합소득금액은 4,500만원 기준).
...
[규칙 3-c — 수치를 물으면 최종 수치까지 산출한다]  ★
질문이 특정 수치(세율·금액·한도)를 물으면 비율이나 구간 설명에서 멈추지 말고
**반드시 최종 수치까지 계산**하십시오. 계산 과정을 단계별로 쓰고, 마지막 줄에
최종 수치를 명시합니다.
  · 이연퇴직소득세에 연차별 감면율을 적용하는 질문이면, 감면율만 제시하고
    끝내지 말고 원래 세율에 감면율을 곱한 **최종 적용 세율**을 산출합니다.
  · 세액공제 금액을 계산할 때 여러 계좌의 납입액을 단순 합산하지 마십시오.
    계좌별 한도(연금저축 단독 한도, 합산 한도)를 각각 적용해 **공제 대상
    금액을 먼저 확정**하고, 거기에 세액공제율을 곱합니다.
...
세 가지로 갈리고, 각각 다르게 답해야 합니다. **이유를 혼동하지 마십시오.**
  ...
  ⓑ 자료의 범위 밖인 경우
     ...
     ⚠ 국민연금 등 공적연금, 다른 금융회사의 상품·수수료·제도는 이 자료의
       범위 밖입니다. 범위 밖 주제는 **먼저 범위 밖임을 밝히고**, 기억에 있는
       일반론으로 서술하지 마십시오. 질문의 나머지 항목은 정상적으로 답합니다.
     ⚠ 근거 목록에 없는 출처(기관명·페이지명·문서명)를 인용하지 마십시오.
       답변 전체에 적용됩니다. 근거에 없는 곳을 "~에 따르면"이라고 쓰면
       확인되지 않은 출처를 지어내는 것입니다.
     ⚠ 펀드를 지목하지 않은 질문에서 수수료·보수 데이터가 없으면
       "요청하신 수수료 정보를 찾지 못했습니다."라고만 답하십시오. 이때도
       검색 근거에 있는 다른 펀드의 수수료 수치를 "참고로" 나열하는 것은
       금지입니다.
...
def compose(state: dict) -> dict:
    ...
    if state.get("sql_empty"):
        user_suffix += (
            ...
            "행이 하나도 없었습니다. 수수료 부분은 다음 문장을 그대로 쓰되 "
            "[펀드명]·[클래스]만 질문에 적힌 표기로 채우십시오:\n"
            "\"요청하신 [펀드명] [클래스] 클래스의 수수료 데이터를 찾지 "
            "못했습니다. 정확한 수치는 해당 펀드 투자설명서나 고객센터를 "
            "통해 확인해 주세요.\"\n"
            "질문이 지목하지 않은 **다른 펀드·클래스의 보수·수수료 수치는 "
            "검색 근거에 있어도 답변에 넣지 마십시오** — '참고로', '다만 "
            "다른 펀드는'처럼 덧붙이는 나열도 금지입니다. 표 조각이나 셀 "
            "참조처럼 보이는 문자열을 보수율인 것처럼 제시하지도 마십시오. "
            "제도 설명이나 해당 펀드의 투자전략·위험등급처럼 수수료가 아닌 "
            "내용은 근거대로 답해도 됩니다.")
        state["trace"].append(
            "compose: fee_sql 0행 → 무관 펀드 수수료 나열 차단 지시")
    ...
    # 개방형 추천 질의면 규칙 5-b를 이 문항에서만 다시 못 박는다.
    # ①만 넣으면 되묻고 끝나는 답변이 나오므로 ②와 한 덩어리로 지시한다.
    _reco = _reco_kind(state["question"])
    if _reco:
        user_suffix += (
            "\n\n※ 이 질문은 상품 추천 요청입니다(조건 "
            + ("일부 제시됨" if _reco == "조건있음" else "미제시") +
            "). 아래 ①과 ②를 **모두** 하십시오. 하나만 하면 실패입니다.\n"
            "① 정확히 답하려면 어떤 정보가 더 필요한지 **되묻는 질문 형태로** "
            "먼저 밝히십시오. 투자성향(원금 손실 감내 정도)·계좌유형(연금저축 / "
            "퇴직연금 IRP·DC)·투자기간·목표금액 중 질문에 없는 것을 물으십시오.\n"
            "② 그 다음 근거 자료로 답할 수 있는 만큼 답하십시오. **되묻고 끝내지 "
            "마십시오** — 이 상담은 한 번의 답변으로 끝나 사용자가 다시 답할 기회가 "
            "없습니다. 근거에 구체적인 상품이나 상품유형(원리금보장상품·예금·ELB·"
            "채권형 등)이 있으면 그 이름을 그대로 적고, 근거에 있는 **위험등급**을 "
            "함께 밝히십시오. 조건이 부족해 하나로 좁힐 수 없으면 성향별로 나눠 "
            "제시하되 무엇에 따라 갈리는지 밝히십시오.")
        if not state.get("sql_rows"):
            user_suffix += (
                "\n③ 보수·수수료의 **구체적인 수치(%)는 제시하지 마십시오.** 이 "
                "질문은 수수료 데이터베이스를 조회하지 않았으므로 검색 근거에 보이는 "
                "숫자는 검증되지 않았습니다. 수수료를 언급해야 하면 수치 없이 "
                "'총보수가 낮은 상품' 같은 상대적 표현만 쓰십시오.")
        state["trace"].append(
            f"compose: 개방형 추천 질의({_reco}) → 규칙 5-b(역질문+답변) 재지시"
            + ("" if state.get("sql_rows") else " · 수수료 수치 제시 금지"))
...
def to_response(state: dict) -> dict:
    ...
    # 추천성 답변에는 투자권유가 아님을 코드로 박는다. 생성에 맡기면
    # 9/9 빠졌다(2026-09-04 실측). 차단 응답에는 붙이지 않는다 — 위
    # _apply_unable_notice와 같은 보호 조건이다.
    if (_ans and not state.get("_blocked")
            and _reco_kind(str(state.get("question") or ""))
            and "투자권유가" not in _ans):
        _ans = _ans.rstrip() + "\n\n" + _RECO_DISCLAIMER
        state["answer"] = _ans
        state.setdefault("trace", []).append(
            "추천 면책: 투자권유가 아님을 코드로 명시")
