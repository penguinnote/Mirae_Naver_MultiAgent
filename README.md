...
> 평가용 엔드포인트: `(배포 후 이 자리에 기입 — http://{공인 IP}/answer)`
...
| **fee_sql** | 자연어를 HCX-007로 SQL로 바꿔 `fund_fees.sqlite`를 조회한다. 생성된 SQL의 **표기 흔들림을 코드에서 흡수**한다 | 아래 별도 설명 |
...
| 판매경로 | `'온라인직접판매'` | `'온라인'`, `'온라인슈퍼'` | `LIKE '온라인%'`로 변환 |
...
| `score_official.py` | 대회 공식 배점(A50/B20/C15/E15)으로 채점 + 실행 간 비교 |
